...
% Ingram
    ...
    % top=0.6in,         % Increased slightly to give the 32pt header breathing room
    % bottom=0.5in,      % Increased slightly for the footer
    % headheight=32pt,   % Sets header height directly in geometry
    % headsep=10pt,      % Space between header and text body
    % footskip=18pt,     % Space from text baseline to footer (increased from 12pt so 10pt font doesn't collide)
    % includehead,       % Counts the header as part of the top margin area
    % includefoot        % Counts the footer as part of the bottom margin area
% KDP, We need to fix the margins, compare with psalterium, its too much top and bottom, which are 6x9. Perhaps 5.25x8? Gutter at 0.75" to 0.875"?
% Consider page count. Do we want 600pg max? or aim for 500? 6x9 is too big, 5x8 is better, but even better is 5x7.5. And 9pt Merriweather is massive, so we can always decrease. Gutter should still be .75. Smaller is better, and less pages better. but <=5x7 is ideal from the below.
% Ascension will be 4.5x6.75in @ 10pt, Ascension Large is 5.5x8.375in @ 12.41pt, WOF will be @ 11.4 and 14pt, Pax inter Spinas is 10.5x16.5cm (very readable), Anthologion is 4.5x7in @ 10pt, current LotH is 4.375x6.75in, large is 5.5x8.375in, (Shorter) Christian Prayer is 4.375x6.75in, large is 5.5x8.125, Angelus (Iris) is 7.16x4.16, Angelus Diurnal is 3.5x5.5 pocket and 4.5x7.5 full, FSSP Diurnal is 4x7in, Nova et Vetera is 11.8x18cm, Baronius is 4.5x7in, ChurchLatin Diurnal is 5.5x8.5", Lancelot Andrews Matins is 5x7.5in, Lancelot Diurnal is 4.25x6.25in, Ukrainian is 5x6.75in (too wide), Before and After 5.75x8.75, Anglican Breviary is 5.75x7.75in
	% paperwidth=5in,
	% paperheight=8in,
	% twoside,
	% inner=0.375in,         % Keeps your visual whitespace to the gutter
	% bindingoffset=0.5in,   % Keeps the "sacrifice zone" for KDP's glue spine
	% outer=0.26in,          % Pushes text right to the edge (safely over KDP's 0.25" floor)
	% top=0.45in,            % Accounts for your 32pt header + 10pt space + 0.25" KDP floor
	% bottom=0.4in,          % Accounts for your footer + 18pt space + 0.25" KDP floor
	% headheight=32pt,       % Your custom tall header height
	% headsep=10pt,          % Space between header and text body
	% footskip=18pt,         % Space from text baseline to footer
% Try these
	paperwidth=5.0in,
...
% \setmainfont{Charis}
% \setmainfont{Amazon Endure Book}
...
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Decorated Roman Initials}}%
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Aneirin}}%
\newcommand*\zallmancaps{\fontspec{Aneirin}}%
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Sentinel}}%oversized
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Gingerbread Initials}}%wrong style
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Napoli Initialen}}%undersized and wrong style
% \newcommand*\zallmancaps{\fontspec[Weight=Bold]{Fabliaux}}%wrong style
% \newcommand*\zallmancaps{\fontspec[Scale=.9]{Erbar Initialen}}%oversized
% \newcommand*\zallmancaps{\fontspec{Zierinitialen}}%faint and not appropriate
% \newcommand*\zallmancaps{\fontspec{Westminster Gotisch}}%like Aneirin but more compact; n overhangs
% \newcommand*\zallmancaps{\fontspec{Lombardic}}%wrong style
% \newcommand*\zallmancaps{\fontspec{Pfeffer Mediæval}}%not appropriate
% \newcommand*\zallmancaps{\fontspec{Celtic Hand}}%ugly
	...
	\begingroup % Start local group to limit scope
		...
		\newcommand{\processline}[1]{%
			...
			\iffirstverse
				...
				\firstversefalse
			\fi
			\hspace*{.1em}% Start the new verse
			\ifblue
...
\setlength{\parindent}{.3em} % Set paragraph indentation
...
% {\normalfont\large\bfseries}{}{0pt}{}
{\normalfont\centering\color{Red}}{}{0pt}{}
...
\pretitle{\begin{center}\bfseries \fontsize{28}{24}\selectfont \color{Red} B R E V I A R I U M\\ \vspace{+.5em}
\fontsize{14}{12}\selectfont \color{black} C U M\\ \vspace{+.75em}}
\title{\fontsize{28}{24}\selectfont \color{Red} P S A L T E R I O \vspace{+.5em}}
\posttitle{\Large \par \itshape \color{black} I U X T A \ \ E C C L E S I A S T I C U M\\ \vspace{+.5em}
\upshape \LARGE \color{Blue} O F F I C I U M \end{center}}
% 5

\preauthor{\vspace{-1em} \begin{center}
\Large {\bfseries \itshape \color{black} SACRI ORDINIS FRATRUM}\\ \vspace{+.5em}
\fontsize{26}{24}\selectfont \bfseries \color{Blue} P R E D I C A T O R U M\\ \vspace{+.5em}}
\author{\color{black} \underline{\color{black} \itshape \large Nova Typographya Restitutum et Impressum}\\ \vspace{+.5em}}
\postauthor{\large \color{Red} \upshape {\Large E}X {\Large E}XEMPLARI {\Large P}ROTHOTYPO\\ \vspace{+.5em}
\end{center}}
% 5

\predate{\vspace{-1em} \begin{center}
\large \itshape {\color{black} Venerabilis Magistri Nostri}\\ \vspace{+.5em}
\upshape \fontsize{26}{22}\selectfont \bfseries \color{Blue} H U M B E R T I\\ \vspace{+.25em}
\Huge \color{Blue} D E \ \ R O M A N I S\\ \vspace{+.5em}}
\date{\color{Red} \large \mdseries \underline{\underline{\color{black} {\Large E}x \normalsize {\Large C}ODICE {\Large R}OME {\Large S}ANCTE {\Large S}ABINE {\Large XIV L1}}}\\ \vspace{+.85em}}
\postdate{\color{Red} \itshape \bfseries EXCERPTUM ET DISPOSITUM\end{center}}
% 4
...
\newline Iuxta Ecclesiasticum Officium
\newline Sacri Ordinis Fratrum
\newline Predicatorum
\end{center}
\begin{center}
...
\newline Digitally edit, transcribed and prepared
\newline for \LaTeX \ by \emph{Alan Liang}
\end{center}
\begin{center}
...
\newline ISBN: 979-8-188456-33-7
\end{center}
\begin{center}

The underlying medieval liturgical text and 
\newline manuscript sources contained in this
\newline volume are in the public domain.
\end{center}
\begin{center}


This diplomatic transcription, liturgical layout, and 
\newline digital compilation are licensed under the
\newline Creative Commons Attribution-ShareAlike 4.0
\newline International License
\newline (CC BY-SA 4.0).
\end{center}
\begin{center}
...
\newline The \LaTeX source code and transcription files
\newline for this work are publicly available and
\newline open for collaboration on GitHub:
...
% removed as superfluous to private recitation
...
% \color{Red} XI & C & \color{Red} III & 3 & Oct. Sancti Iohannis. & \color{Red} Incipit Quartus Embolismus.\\
...
% 1909 has him on Jan 18, XV until the Kalends of Feb. Did they misread IIII has XV?
...
% \color{Red} XVII & F & \color{Red} VI & 27 & \color{Red} Iuliani Episcopi et Confessoris. Mem. \\
...
% \color{Red} XIX & D & \color{Red} V & 3 & Inventio Sancte Crucis. Semid. \\
 % &  &  &  & \multicolumn{2}{l}{\qquad Alexandri Eventii, et Theodoli. \color{Red} Martyrum. \color{black} Mem.} \\
...
& & & & \qquad Sepultorum in Cimiteriis Nostri. & \color{Red} \\
...
\lettrine[lines=2]{\zallmancaps \color{Blue} D}{}\ul{\textsc{um} dicitur psalmus} \psalm{148} \ul{in Matutinis: accedit qui lecturus est Kalendas ad prelatum, dicens ei summisse: \emph{Capitulum?} Qui si dixerit \emph{Non}: legende erunt Kalende in choro, finitis Matutinis. Si autem dixerit \emph{Post Primam}: tunc reservande sunt usque ad Primam. Si autem dixerit \emph{Sic}: legende erunt in capitulo post Matutinas.}
...
\newline \ul{Cum autem reservantur usque post Primam, dum dicitur ultimus psalmus in Prima: accedat qui legere debet ad prelatum, interrogans eum ut supra. Etsi dixerit \emph{Sic}: tunc cito pulsetur ad capitulum ante terminationem hore, et omnia faciat ut supra, preterquam de luminibus, si sit tanta dies quod non sit lumen necessarium. Si autem dixerit \emph{Non}: legende sunt in choro. Cum autem in choro leguntur sive post Matutinas sive post Primam: non pulsatur capitulum, sed alia fiunt modo supradicto, prout competunt.}
...
% \newline \ul{Si vero tabula legenda fuerit. Legatur ante recitationem obitus vel anniversarii, et antequam dicatur} Commemoratio.
...
% Liber iste qui collectarius dicitur eo quo propter collectas sive orationes habendas in promptu principaliter scriptus est, vel quia in unum sunt in eo collecta ea que dicenda sunt a sacerdote extra officium Misse: continent ista per ordinem.
